...
fn benchmark_serialization(c: &mut Criterion) {
    ...
    group.bench_function("health_response_serialize", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::to_string(black_box(&response)).unwrap())
        });
    });
    ...
    group.bench_function("health_response_deserialize", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::from_str::<HealthResponse>(black_box(json)).unwrap())
        });
    });
    ...
    group.bench_function("security_headers", |b| {
        ...
        b.iter(|| {
            let headers = vec![
                ...
                ("X-XSS-Protection", HeaderValue::from_static("1; mode=block")),
            ];
            black_box(headers)
        });
    });
...
fn benchmark_s3_operations(c: &mut Criterion) {
    ...
    group.bench_function("json_serialization", |b| {
        ...
        b.iter(|| {
            black_box(serde_json::to_vec(black_box(&data)).unwrap())
        });
    });
