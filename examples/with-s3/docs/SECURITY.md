...
Security best practices and hardening guide for example-app.
...
- apiGroups: [""]
  resources: ["secrets"]
  verbs: ["get"]
  resourceNames: ["example-app-secrets"]
```
...
spec:
  ...
  - to:
    - namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: example-app
    ports:
    - protocol: TCP
      port: 6379  # Redis
...
metadata:
  name: example-app
  annotations:
    eks.amazonaws.com/role-arn: arn:aws:iam::ACCOUNT_ID:role/example-app-role
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:REGION:ACCOUNT_ID:secret:/example-app/*"
    }
  ]
}
```
...
aws secretsmanager update-secret \
  --secret-id /example-app/production/redis-url \
