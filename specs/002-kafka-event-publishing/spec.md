...
- What happens when Kafka publishing is disabled? The service simply does not include Kafka publishing capabilities; handlers that would publish events are not generated in the template (conditional Liquid template logic).

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The template MUST prompt users during generation about enabling Kafka event publishing (conditional feature like S3 and PostgreSQL).
- **FR-002**: When Kafka publishing is enabled, the template MUST prompt for: broker URL (e.g., "kafka.kafka.svc.cluster.local:9092"), topic name (e.g., "events"), and event name (e.g., "service.event.published").
- **FR-003**: The generated project MUST include Kafka client libraries and publishing dependencies only when the feature is enabled.
- **FR-004**: When Kafka publishing is enabled, the template MUST generate HTTP handlers (or handler implementations) that publish dummy CloudEvents-formatted events to the configured Kafka topic using conditional Liquid template logic.
...
- **FR-009**: When Kafka publishing is disabled, the template MUST not include Kafka dependencies, publishing code, or handlers that publish events (determined by conditional Liquid template logic).
- **FR-010**: Configuration validation MUST occur at startup; the service MUST fail fast if Kafka publishing is enabled but broker URL or topic name are empty.
