...
spec:
  template:
    spec:
      containers:
       - image: {{ image_registry }}/{{ github_org }}/{{ project_name }}:latest
        env:
...
1. **Build and push image**:
   ```bash
    docker build -t {{ image_registry }}/{{ github_org }}/{{ project_name }}:v1.0 .
    docker push {{ image_registry }}/{{ github_org }}/{{ project_name }}:v1.0
   ```
