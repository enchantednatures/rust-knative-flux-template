...
Guide for local development, adding features, and following best practices for {{ project_name }}.
...
# Log specific module
RUST_LOG={{ crate_name }}=debug cargo run
...
```bash
# Create worktree
git worktree add ../{{ project_name }}-feature feature/my-feature
...
# Cleanup when done
cd ..
git worktree remove ../{{ project_name }}-feature
```
