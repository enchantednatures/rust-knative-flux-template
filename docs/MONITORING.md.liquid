...
3. Select Operation: `/api/upload`{% if features contains "s3" %}, `/api/download`, etc.{% endif %}
4. Click "Find Traces"
...
- **Business**: Requests served, errors{% if features contains "s3" %}, objects uploaded{% endif %}
  ...
  - name: errors
    ...
    - alert: HighErrorRate
      ...
      annotations:
        ...
        description: "Error rate is {% raw %}{{ $value }}{% endraw %} errors/sec for the last 5 minutes"
      labels:
        severity: warning
```
...
```yaml
    - alert: PodNotReady
      ...
      annotations:
        ...
        description: "{% raw %}{{ $value }}{% endraw %} pods not ready"
      labels:
        severity: critical
```
...
{% if features contains "s3" %}8. **S3 Upload Rate**: `rate(s3_upload_total[5m])`
...
{% if features contains "s3" %}- S3 operation success rate{% endif %}
...
2. Review error logs
3. Check dependencies (Redis{% if features contains "s3" %}, S3{% endif %})
