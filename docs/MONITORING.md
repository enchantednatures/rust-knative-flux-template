...
3. Select Operation: `/api/upload`{% if feature_s3 %}, `/api/download`, etc.{% endif %}
4. Click "Find Traces"
...
- **Business**: Requests served, errors{% if feature_s3 %}, objects uploaded{% endif %}
...
{% if feature_s3 %}8. **S3 Upload Rate**: `rate(s3_upload_total[5m])`
...
{% if feature_s3 %}- S3 operation success rate{% endif %}
...
2. Review error logs
3. Check dependencies (Redis{% if feature_s3 %}, S3{% endif %})
