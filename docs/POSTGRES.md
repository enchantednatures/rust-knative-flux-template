...
./scripts/dev/restore-from-backup.sh -b postgres-app-backup-1704326400
...
3. **Verify deployment**:
   ```bash
   kubectl get cluster postgres-app -o wide
   kubectl get pods -l postgresql=postgres-app -o wide
   ```
...
kubectl get pods -l postgresql=postgres-app

# Streaming replication status
kubectl exec postgres-app-1 -- psql -U postgres -c "SELECT * FROM pg_stat_replication;"
```
...
kubectl patch cluster postgres-app --type='json' -p='[{"op": "replace", "path": "/spec/instances", "value":2}]'

# Verify scaling
kubectl get pods -l postgresql=postgres-app --watch
```
...
kubectl logs postgres-app-1 | grep -i "failover\|promote"
```
...
kubectl rollout restart statefulset postgres-app
```
...
# Check pod events
kubectl describe pod postgres-app-1
...
kubectl patch pvc postgres-app-1 -p '{"spec":{"resources":{"requests":{"storage":"50Gi"}}}}'
```
...
watch -n 5 'kubectl exec postgres-app-1 -- psql -U postgres -c "SELECT client_addr, write_lag, flush_lag, replay_lag FROM pg_stat_replication;"'
...
# For MinIO:
kubectl exec postgres-app-1 -- s3cmd ls s3://postgres-backups/
...
kubectl exec -it postgres-app-1 -- psql -U postgres -d postgres -c "SELECT 1;"

# Check service endpoints
kubectl get endpoints postgres-app-rw
kubectl get endpoints postgres-app-ro
...
kubectl exec postgres-app-1 -- psql -U postgres -d app -c "SELECT * FROM pg_stat_statements ORDER BY mean_exec_time DESC LIMIT 10;"

# Check table sizes
kubectl exec postgres-app-1 -- psql -U postgres -d app -c "SELECT tablename, pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) FROM pg_tables WHERE schemaname='public' ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC;"

# Check index usage
kubectl exec postgres-app-1 -- psql -U postgres -d app -c "SELECT * FROM pg_stat_user_indexes WHERE idx_scan = 0;"
```
...
kubectl exec postgres-app-1 -- psql -U postgres -c "SELECT * FROM pg_stat_archiver;"
...
kubectl exec postgres-app-1 -- psql -U postgres -c "SELECT * FROM pg_create_physical_replication_slot('slot_name');"

# Monitor replication slot
kubectl exec postgres-app-1 -- psql -U postgres -c "SELECT * FROM pg_stat_replication_slots;"
```
