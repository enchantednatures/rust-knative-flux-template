...
Security best practices and hardening guide for {{ project_name }}.
...
# Scan image
trivy image {{ image_registry }}/{{ github_org }}/{{ project_name }}:latest

# Scan with severity filter
trivy image --severity HIGH,CRITICAL {{ image_registry }}/{{ github_org }}/{{ project_name }}:latest
```
...
# Scan image
grype {{ image_registry }}/{{ github_org }}/{{ project_name }}:latest
```
...
# Sign image
cosign sign --key cosign.key {{ image_registry }}/{{ github_org }}/{{ project_name }}:v1.0.0

# Verify signature
cosign verify --key cosign.pub {{ image_registry }}/{{ github_org }}/{{ project_name }}:v1.0.0
...
- apiGroups: [""]
  resources: ["secrets"]
  verbs: ["get"]
  resourceNames: ["{{ project_name }}-secrets"]
```
...
metadata:
  name: {{ project_name }}
  annotations:
    eks.amazonaws.com/role-arn: arn:aws:iam::ACCOUNT_ID:role/{{ project_name }}-role
```
...
aws secretsmanager update-secret \
  --secret-id /{{ project_name }}/production/redis-url \
