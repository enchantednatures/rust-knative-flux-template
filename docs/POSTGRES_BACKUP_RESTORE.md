...
# Backup job created: postgres-app-backup-1704326400
...
kind: Backup
metadata:
  name: postgres-app-backup-before-upgrade
  namespace: default
spec:
  cluster:
    name: postgres-app
...
# Monitor
kubectl get backup postgres-app-backup-before-upgrade -w
kubectl describe backup postgres-app-backup-before-upgrade
```
...
# postgres-app-daily-backup      0 2 * * *    postgres-app    False
#
# === PostgreSQL Manual Backups ===
# NAME                                  PHASE       STARTTIME             ENDTIME
# postgres-app-backup-1704326400       completed   2024-01-03T16:30:45Z  2024-01-03T16:42:22Z
```
...
kubectl get schedulebackup postgres-app-daily-backup -o jsonpath='{.status.lastScheduleTime}'
...
kubectl patch schedulebackup postgres-app-daily-backup \
  -p '{"spec":{"schedule":"0 1 * * *"}}'
...
# Restore from specific backup
./scripts/dev/restore-from-backup.sh \
  -b postgres-app-backup-1704326400 \
...
kubectl exec postgres-app-1 -- psql -U postgres -d app -c "REINDEX INDEX CONCURRENTLY idx_name;"
```
...
kubectl exec postgres-app-1 -- psql -U postgres -d app -c "SELECT COUNT(*) FROM my_table;"
...
kubectl get backup postgres-app-backup-1704326400 -o yaml | grep -E "checksum|phase|size"

# Check WAL file integrity
kubectl exec postgres-app-1 -- pg_checkpoints
```
...
kubectl logs postgres-app-backup-pod

# Kill stuck backup (forces retry)
kubectl delete backup postgres-app-backup-<name>
```
