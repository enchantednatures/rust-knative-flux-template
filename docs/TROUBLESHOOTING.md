...
Common issues, error messages, and solutions for {{ project_name }}.
...
4. Check conditions:
```bash
kubectl get ksvc {{ project_name }} -n {{ project_name }} -o jsonpath='{.status.conditions}'
```
...
```
Failed to pull image "{{ image_registry }}/{{ github_org }}/{{ project_name }}:latest":
rpc error: code = Unknown desc = Error pulling image: unauthorized
```

3. Verify registry credentials:
```bash
kubectl get secret registry-credentials -n {{ project_name }} -o yaml
```
...
```bash
kubectl set image deployment/{{ project_name }} user-container={{ image_registry }}/{{ github_org }}/{{ project_name }}:latest
```
...
```bash
kubectl logs <pod-name> -n {{ project_name }} --previous
```
...
```bash
kubectl edit deployment {{ project_name }} -n {{ project_name }}
...
```bash
kubectl get configmaps -n {{ project_name }}
```
...
```bash
kubectl describe deployment {{ project_name }} -n {{ project_name }}
# Check env: section
```
...
kubectl run debug --image=busybox --rm -it --restart=Never \
  -n {{ project_name }} -- sh -c "nc -zv redis 6379"
```
...
```bash
kubectl get secret {{ project_name }}-secrets -n {{ project_name }} -o yaml
```
...
```bash
kubectl get pod <pod-name> -n {{ project_name }} -o jsonpath='{.spec.containers[0].resources.limits}'
```
...
```bash
kubectl top pod <pod-name> -n {{ project_name }}
```
...
```bash
kubectl describe gitrepository {{ project_name }} -n {{ project_name }}
```

2. Check conditions:
```bash
kubectl get gitrepository {{ project_name }} -n {{ project_name }} -o jsonpath='{.status.conditions}'
```
...
4. Sync manually:
```bash
flux reconcile source git {{ project_name }} -n {{ project_name }}
```
...
```bash
kubectl describe kustomization {{ project_name }} -n {{ project_name }}
```

2. Check error message:
```bash
kubectl get kustomization {{ project_name }} -n {{ project_name }} -o yaml | grep -A 10 status
```
...
```bash
kubectl get imagerepository {{ project_name }} -n {{ project_name }}
```

2. Check ImagePolicy:
```bash
kubectl get imagepolicy {{ project_name }} -n {{ project_name }}
```
...
```bash
flux get image update {{ project_name }} -n {{ project_name }}
```

5. Manually reconcile:
```bash
flux reconcile image update {{ project_name }} -n {{ project_name }}
```
...
```bash
kubectl get pod <pod-name> -n {{ project_name }} -o jsonpath='{.spec.containers[0].resources}'
```
...
# Kubernetes
kubectl set env deployment/{{ project_name }} RUST_LOG=debug -n {{ project_name }}

# Specific module
RUST_LOG={{ crate_name }}=debug,redis=trace cargo run
```
...
# In Kubernetes
kubectl set env deployment/{{ project_name }} RUST_BACKTRACE=1
```
...
kubectl port-forward <pod-name> 8080:8080 -n {{ project_name }}
...
```bash
# Start shell in pod
kubectl exec -it <pod-name> -n {{ project_name }} -- sh
...
```
# GitHub Issues
https://github.com/your-org/{{ project_name }}/issues

# Stack Overflow
https://stackoverflow.com/search?q={{ project_name }}
```
...
# Service status
kubectl get ksvc {{ project_name }} -n {{ project_name }} -o yaml

# Pod status
kubectl get pods -n {{ project_name }} -l serving.knative.dev/service={{ project_name }} -o yaml
...
# ConfigMaps
kubectl get configmaps -n {{ project_name }} -o yaml

# Secrets (sanitized)
kubectl get secrets -n {{ project_name }} -o yaml | grep -v password: | grep -v token:
EOF
```
