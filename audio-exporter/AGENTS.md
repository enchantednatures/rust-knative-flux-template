...
- **Makefile.liquid**: Templated with `PROJECT_NAME` (hyphens) and `CRATE_NAME` (underscores)
  - `PROJECT_NAME := audio-exporter`
  - `CRATE_NAME := audio_exporter`

- **scripts/dev/build-and-deploy.sh.liquid**: Hardcoded service and binary names at generation time
  - `SERVICE_NAME="audio-exporter"`  (for Docker tags, Kubernetes)
  - `CRATE_NAME="audio_exporter"`  (for binary verification)

- **deploy/base/knative-service.yaml.liquid**: Uses `audio-exporter` for service name
  - `name: audio-exporter`
