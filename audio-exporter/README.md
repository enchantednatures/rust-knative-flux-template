...
- 🧪 **Fully Tested**: Integration tests with Redis, MinIO
...
4. **Configure S3 Credentials**
   ```bash
   kubectl create secret generic audio-exporter-s3 \
...
│   ├── state.rs          # AppState with Redis, storage Operator
...
- `APP__TELEMETRY__SERVICE_NAME`: Service name for traces (default: audio-exporter)
...
# Build Docker image
docker build -t audio-exporter:latest .
...
kubectl describe ksvc audio-exporter

# Check pods
kubectl get pods -l serving.knative.dev/service=audio-exporter
...
4. Review service logs: `kubectl logs -f svc/audio-exporter`
