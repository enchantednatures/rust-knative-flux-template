...
Common issues, error messages, and solutions for audio-exporter.
...
```
Error: Connection refused (os error 61)
Error: S3 operation failed: Connection refused
```
...
```bash
make dev-status
# Should show minio pod in "services" namespace
```

2. Restart MinIO (part of make dev-up):
```bash
make dev-down
make dev-up
```
...
4. Test MinIO health via port-forward (from another terminal):
```bash
make dev-forward
...
5. Verify MinIO credentials:
```bash
echo $AWS_ACCESS_KEY_ID
echo $AWS_SECRET_ACCESS_KEY
# Expected: minioadmin / minioadmin (default)
```

6. Check MinIO console:
- Start port forwarding: `make dev-forward`
...
```
Error: NoSuchBucket
Error: S3 operation failed: Bucket not found
```
...
```bash
cat config/development.toml | grep bucket
# Should show: bucket = "data"
```

4. Use Terraform to create bucket:
```bash
cd terraform
terraform apply
```
...
**Symptoms**:
```
kubectl get ksvc audio-exporter
# Shows: Ready: False
```
...
1. Describe service:
```bash
kubectl describe ksvc audio-exporter -n audio-exporter
```
...
4. Check conditions:
```bash
kubectl get ksvc audio-exporter -n audio-exporter -o jsonpath='{.status.conditions}'
```
...
1. Describe pod:
```bash
kubectl describe pod <pod-name> -n audio-exporter
```

2. Check error message:
```
Failed to pull image "ghcr.io/enchantednatures/audio-exporter:latest":
rpc error: code = Unknown desc = Error pulling image: unauthorized
```

3. Verify registry credentials:
```bash
kubectl get secret registry-credentials -n audio-exporter -o yaml
```
...
```bash
kubectl set image deployment/audio-exporter user-container=ghcr.io/enchantednatures/audio-exporter:latest
```
...
1. View pod logs:
```bash
kubectl logs <pod-name> -n audio-exporter
```

2. View previous container logs:
```bash
kubectl logs <pod-name> -n audio-exporter --previous
```
...
```bash
kubectl edit deployment audio-exporter -n audio-exporter
...
```bash
kubectl get configmaps -n audio-exporter
```

2. Check if Secret exists:
```bash
kubectl get secrets -n audio-exporter
```
...
```bash
kubectl describe deployment audio-exporter -n audio-exporter
# Check env: section
```
...
kubectl run debug --image=busybox --rm -it --restart=Never \
  -n audio-exporter -- sh -c "nc -zv redis 6379"
```
...
```
Error: Access Denied
Error: S3 operation failed: AccessDenied
```
...
```bash
kubectl get secret audio-exporter-secrets -n audio-exporter -o yaml
```

3. Verify IAM permissions (AWS S3):
```bash
...
```bash
kubectl get pod <pod-name> -n audio-exporter -o jsonpath='{.spec.containers[0].resources.limits}'
```
...
```bash
kubectl top pod <pod-name> -n audio-exporter
```
...
**Symptoms**:
```
kubectl get gitrepository -n audio-exporter
# Shows: Ready: False
```
...
```bash
kubectl describe gitrepository audio-exporter -n audio-exporter
```

2. Check conditions:
```bash
kubectl get gitrepository audio-exporter -n audio-exporter -o jsonpath='{.status.conditions}'
```
...
4. Sync manually:
```bash
flux reconcile source git audio-exporter -n audio-exporter
```
...
**Symptoms**:
```
kubectl get kustomization -n audio-exporter
# Shows: Ready: False
```
...
```bash
kubectl describe kustomization audio-exporter -n audio-exporter
```

2. Check error message:
```bash
kubectl get kustomization audio-exporter -n audio-exporter -o yaml | grep -A 10 status
```
...
5. Sync manually:
```bash
flux reconcile kustomization audio-exporter -n audio-exporter
```
...
```bash
kubectl get imagerepository audio-exporter -n audio-exporter
```

2. Check ImagePolicy:
```bash
kubectl get imagepolicy audio-exporter -n audio-exporter
```
...
```bash
flux get image update audio-exporter -n audio-exporter
```

5. Manually reconcile:
```bash
flux reconcile image update audio-exporter -n audio-exporter
```
...
```bash
kubectl get pod <pod-name> -n audio-exporter -o jsonpath='{.spec.containers[0].resources}'
```
...
# Kubernetes
kubectl set env deployment/audio-exporter RUST_LOG=debug -n audio-exporter

# Specific module
RUST_LOG=audio_exporter=debug,redis=trace cargo run
```
...
# In Kubernetes
kubectl set env deployment/audio-exporter RUST_BACKTRACE=1
```
...
kubectl port-forward <pod-name> 8080:8080 -n audio-exporter
...
```bash
# Start shell in pod
kubectl exec -it <pod-name> -n audio-exporter -- sh
...
```
# GitHub Issues
https://github.com/your-org/audio-exporter/issues

# Stack Overflow
https://stackoverflow.com/search?q=audio-exporter
```
...
# Service status
kubectl get ksvc audio-exporter -n audio-exporter -o yaml

# Pod status
kubectl get pods -n audio-exporter -l serving.knative.dev/service=audio-exporter -o yaml

# Service logs
kubectl logs deployment/audio-exporter -n audio-exporter

# FluxCD status
flux get kustomizations -n audio-exporter

# ConfigMaps
kubectl get configmaps -n audio-exporter -o yaml

# Secrets (sanitized)
kubectl get secrets -n audio-exporter -o yaml | grep -v password: | grep -v token:
EOF
```
