...
1. **postgres-metrics**: Scrapes metrics from PostgreSQL pods in the `default` namespace
   - Target: Pods with `postgresql=audio-exporter-postgres` label
   - Port: `metrics` (9187)
   - Interval: 30 seconds
   - Scheme: HTTP
...
kubectl port-forward -n default svc/audio-exporter-postgres-rw 9187:9187
...
3. **Verify storage connectivity**:
   ```bash
   # MinIO example
   kubectl exec -it audio-exporter-postgres-1 -- \
...
1. **Check replica status**: `kubectl describe cluster audio-exporter-postgres`
2. **Check replica logs**: `kubectl logs audio-exporter-postgres-2`
...
1. **Check pod status**: `kubectl get pod audio-exporter-postgres-N -o wide`
2. **Check pod events**: `kubectl describe pod audio-exporter-postgres-N`
3. **Review pod logs**: `kubectl logs audio-exporter-postgres-N`
