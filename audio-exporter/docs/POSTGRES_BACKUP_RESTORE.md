...
# Output:
# Creating manual backup: audio-exporter-postgres-backup-1704326400
# Backup job created: audio-exporter-postgres-backup-1704326400
...
kind: Backup
metadata:
  name: audio-exporter-postgres-backup-before-upgrade
  namespace: default
spec:
  cluster:
    name: audio-exporter-postgres
  method: plugin
  pluginConfiguration:
...
# Monitor
kubectl get backup audio-exporter-postgres-backup-before-upgrade -w
kubectl describe backup audio-exporter-postgres-backup-before-upgrade
```
...
# audio-exporter-postgres-daily-backup      0 2 * * *    audio-exporter-postgres    False
#
# === PostgreSQL Manual Backups ===
# NAME                                  PHASE       STARTTIME             ENDTIME
# audio-exporter-postgres-backup-1704326400       completed   2024-01-03T16:30:45Z  2024-01-03T16:42:22Z
```
...
kubectl get schedulebackup audio-exporter-postgres-daily-backup -o jsonpath='{.status.lastScheduleTime}'
...
kubectl patch schedulebackup audio-exporter-postgres-daily-backup \
  -p '{"spec":{"schedule":"0 1 * * *"}}'
...
# Restore from specific backup
./scripts/dev/restore-from-backup.sh \
  -b audio-exporter-postgres-backup-1704326400 \
...
kubectl exec audio-exporter-postgres-1 -- psql -U postgres -d app -c "REINDEX INDEX CONCURRENTLY idx_name;"
```
...
kubectl exec audio-exporter-postgres-1 -- psql -U postgres -d app -c "SELECT COUNT(*) FROM my_table;"
...
kubectl get backup audio-exporter-postgres-backup-1704326400 -o yaml | grep -E "checksum|phase|size"

# Check WAL file integrity
kubectl exec audio-exporter-postgres-1 -- pg_checkpoints
```
...
kubectl logs audio-exporter-postgres-backup-pod

# Kill stuck backup (forces retry)
kubectl delete backup audio-exporter-postgres-backup-<name>
```
