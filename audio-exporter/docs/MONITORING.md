...
Complete observability guide for audio-exporter, including distributed tracing, metrics, and logs.
...
2. Select Service: `audio-exporter`
...
```promql
# Upload rate
rate(s3_upload_total[5m])

# Download latency
histogram_quantile(0.99, s3_download_duration_seconds_bucket)
...
impl MyMetrics {
    pub fn new() -> Self {
        let meter = global::meter("audio_exporter");
...
```json
{
  ...
  "level": "INFO",
  "target": "audio_exporter::handlers::api",
...
# Filter by module
RUST_LOG=audio_exporter=debug,redis=info cargo run
...
# Multiple pods
kubectl logs -f -n audio-exporter -l serving.knative.dev/service=audio-exporter

# Previous revision
kubectl logs -f deployment/audio-exporter --previous -n audio-exporter

# Last 100 lines
kubectl logs --tail=100 deployment/audio-exporter -n audio-exporter

# Filter by trace ID
kubectl logs deployment/audio-exporter -n audio-exporter | grep trace-abc123
```

**Structured Log Query (jq)**:
```bash
# Find errors
kubectl logs deployment/audio-exporter -n audio-exporter | jq 'select(.level == "ERROR")'

# Find slow requests (> 1s)
kubectl logs deployment/audio-exporter -n audio-exporter | jq 'select(.http.duration_ms > 1000)'

# Group by path
kubectl logs deployment/audio-exporter -n audio-exporter | jq -r '.http.path' | sort | uniq -c
```
...
spec:
  ...
  - name: errors
    ...
    - alert: HighErrorRate
      ...
      for: 5m
      annotations:
        summary: "High error rate on audio-exporter"
...
```yaml
    - alert: HighLatency
      ...
      for: 5m
      annotations:
        summary: "High latency on audio-exporter"
...
{
  "dashboard": {
    "title": "audio-exporter Dashboard",
    "panels": [
      {
        "title": "Request Rate",
        "targets": [
