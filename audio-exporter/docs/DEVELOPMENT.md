...
Guide for local development, adding features, and following best practices for audio-exporter.
...
| Kind | 0.17+ | https://kind.sigs.k8s.io/docs/user/quick-start/ |
...
```bash
# Start all services (Kind cluster with Knative, Redis, MinIO, Jaeger, Prometheus, OTEL Collector)
make dev-up
...
# 3. Run with auto-reload (in another terminal)
cargo watch -q -c -w src -x run

# 4. Set up port forwarding (in another terminal)
make dev-forward
...
### Using Kind for Integration Testing
...
# Log specific module
RUST_LOG=audio_exporter=debug cargo run
...
# Run with debugger
rust-lldb target/debug/audio-exporter
...
```bash
# Create worktree
git worktree add ../audio-exporter-feature feature/my-feature

# Navigate to worktree
cd ../audio-exporter-feature

# Make changes, commit

# Cleanup when done
cd ..
git worktree remove ../audio-exporter-feature
```
