...
# Project names are normalized at template generation time
# SERVICE_NAME uses hyphens (Kubernetes convention)
SERVICE_NAME="audio-exporter"
# CRATE_NAME uses underscores (Rust convention)  
CRATE_NAME="audio_exporter"
...
# Verify the crate name matches what we expect
ACTUAL_CRATE=$(grep '^name = ' "${PROJECT_ROOT}/Cargo.toml" | head -1 | sed 's/name = "\(.*\)"/\1/')
if [[ "${ACTUAL_CRATE}" != "${CRATE_NAME}" ]]; then
	echo -e "${RED}✗ Error: Crate name mismatch${NC}"
	echo "Expected: ${CRATE_NAME}"
	echo "Found: ${ACTUAL_CRATE}"
	exit 1
fi
...
if ! kubectl wait --for=condition=Ready ksvc/"${SERVICE_NAME}" -n default --timeout=5m; then
	echo -e "${RED}✗ Error: Knative service failed to become ready${NC}"
	echo ""
	echo "Service status:"
	kubectl get ksvc "${SERVICE_NAME}" -n default -o yaml
	echo ""
	echo "Pod status:"
	kubectl get pods -n default
	echo ""
	echo "Recent logs:"
	kubectl logs -l serving.knative.dev/service="${SERVICE_NAME}" -c user-container -n default --tail=50 2>/dev/null || echo "No logs available"
	exit 1
fi
...
# Get service URL
SERVICE_URL=$(kubectl get ksvc "${SERVICE_NAME}" -n default -o jsonpath='{.status.url}' 2>/dev/null || echo "")
...
kubectl get ksvc "${SERVICE_NAME}" -n default
echo ""
