...
# Or via kubectl
kubectl exec -it test-app-postgres-1 -- psql -U app -d app
```
...
./scripts/dev/restore-from-backup.sh -b test-app-postgres-backup-1704326400
...
3. **Verify deployment**:
   ```bash
   kubectl get cluster test-app-postgres -o wide
   kubectl get pods -l postgresql=test-app-postgres -o wide
   ```
...
kubectl get cluster test-app-postgres

# Detailed status
kubectl describe cluster test-app-postgres

# Check instance status
kubectl get pods -l postgresql=test-app-postgres

# Streaming replication status
kubectl exec test-app-postgres-1 -- psql -U postgres -c "SELECT * FROM pg_stat_replication;"
```
...
kubectl patch cluster test-app-postgres --type='json' -p='[{"op": "replace", "path": "/spec/instances", "value":2}]'

# Verify scaling
kubectl get pods -l postgresql=test-app-postgres --watch
```
...
kubectl logs test-app-postgres-1 | grep -i "failover\|promote"
```
...
kubectl delete pod test-app-postgres-1
...
kubectl rollout restart statefulset test-app-postgres
```
...
# Check pod events
kubectl describe pod test-app-postgres-1

# Check pod logs
kubectl logs test-app-postgres-1
```
...
kubectl patch pvc test-app-postgres-1 -p '{"spec":{"resources":{"requests":{"storage":"50Gi"}}}}'
```
...
watch -n 5 'kubectl exec test-app-postgres-1 -- psql -U postgres -c "SELECT client_addr, write_lag, flush_lag, replay_lag FROM pg_stat_replication;"'
...
# For MinIO:
kubectl exec test-app-postgres-1 -- s3cmd ls s3://postgres-backups/
...
kubectl exec -it test-app-postgres-1 -- psql -U postgres -d postgres -c "SELECT 1;"

# Check service endpoints
kubectl get endpoints test-app-postgres-rw
kubectl get endpoints test-app-postgres-ro
...
kubectl exec test-app-postgres-1 -- psql -U postgres -d app -c "SELECT * FROM pg_stat_statements ORDER BY mean_exec_time DESC LIMIT 10;"

# Check table sizes
kubectl exec test-app-postgres-1 -- psql -U postgres -d app -c "SELECT tablename, pg_size_pretty(pg_total_relation_size(schemaname||'.'||tablename)) FROM pg_tables WHERE schemaname='public' ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC;"

# Check index usage
kubectl exec test-app-postgres-1 -- psql -U postgres -d app -c "SELECT * FROM pg_stat_user_indexes WHERE idx_scan = 0;"
```
...
kubectl exec test-app-postgres-1 -- psql -U postgres -c "SELECT * FROM pg_stat_archiver;"
...
kubectl exec test-app-postgres-1 -- psql -U postgres -c "SELECT * FROM pg_create_physical_replication_slot('slot_name');"

# Monitor replication slot
kubectl exec test-app-postgres-1 -- psql -U postgres -c "SELECT * FROM pg_stat_replication_slots;"
```
