...
Guide for local development, adding features, and following best practices for test-app.
...
```bash
# Start all services (Kind cluster with Knative, Redis, Jaeger, Prometheus, OTEL Collector)
make dev-up
...
# Log specific module
RUST_LOG=test_app_no_s3=debug cargo run
...
# Run with debugger
rust-lldb target/debug/test-app
...
```bash
# Create worktree
git worktree add ../test-app-feature feature/my-feature
...
# Cleanup when done
cd ..
git worktree remove ../test-app-feature
```
