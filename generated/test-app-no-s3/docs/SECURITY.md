...
Security best practices and hardening guide for test-app.
...
COPY --from=builder /app/target/release/test-app /usr/local/bin/test-app
...
# Scan image
trivy image localhost:5001/test-org/test-app:latest

# Scan with severity filter
trivy image --severity HIGH,CRITICAL localhost:5001/test-org/test-app:latest
```
...
# Scan image
grype localhost:5001/test-org/test-app:latest
```
...
# Sign image
cosign sign --key cosign.key localhost:5001/test-org/test-app:v1.0.0

# Verify signature
cosign verify --key cosign.pub localhost:5001/test-org/test-app:v1.0.0
...
- apiGroups: [""]
  resources: ["secrets"]
  verbs: ["get"]
  resourceNames: ["test-app-secrets"]
```
...
metadata:
  name: test-app
  annotations:
    eks.amazonaws.com/role-arn: arn:aws:iam::ACCOUNT_ID:role/test-app-role
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:REGION:ACCOUNT_ID:secret:/test-app/*"
    }
  ]
}
```
...
aws secretsmanager update-secret \
  --secret-id /test-app/production/redis-url \
