...
1. **postgres-metrics**: Scrapes metrics from PostgreSQL pods in the `default` namespace
   - Target: Pods with `postgresql=test-app-postgres` label
   - Port: `metrics` (9187)
   - Interval: 30 seconds
   - Scheme: HTTP
...
kubectl port-forward -n default svc/test-app-postgres-rw 9187:9187
...
3. **Verify storage connectivity**:
   ```bash
   # MinIO example
   kubectl exec -it test-app-postgres-1 -- \
...
1. **Check replica status**: `kubectl describe cluster test-app-postgres`
2. **Check replica logs**: `kubectl logs test-app-postgres-2`
...
1. **Check pod status**: `kubectl get pod test-app-postgres-N -o wide`
2. **Check pod events**: `kubectl describe pod test-app-postgres-N`
3. **Review pod logs**: `kubectl logs test-app-postgres-N`
