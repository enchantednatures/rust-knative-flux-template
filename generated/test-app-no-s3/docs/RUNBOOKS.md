...
Procedures for operating test-app in production.
...
kubectl get ksvc test-app -n test-app
```
...
# Slack: #incident-test-app-TIMESTAMP
...
kubectl scale deployment/test-app --replicas=20 -n test-app

# Kill slow pods
kubectl delete pod <pod-name> -n test-app

# Disable canary (shift all traffic to stable)
kubectl set traffic test-app --revision=test-app-stable --percent=100
```
...
docker build -t localhost:5001/test-org/test-app:fix-v1 .
docker push localhost:5001/test-org/test-app:fix-v1

# Deploy (via GitOps or manual)
kubectl set image deployment/test-app user-container=localhost:5001/test-org/test-app:fix-v1 -n test-app
...
```bash
# Edit ConfigMap
kubectl edit configmap test-app-config -n test-app

# Edit Secret
kubectl edit secret test-app-secrets -n test-app
...
**Template**:
```markdown
# Post-Mortem: test-app Incident [Date]
...
```bash
kubectl edit ksvc test-app -n test-app
```
...
kubectl patch ksvc test-app -n test-app -p '{"spec":{"template":{"metadata":{"annotations":{"autoscaling.knative.dev/minScale":"50"}}}}}'

# After event, reduce
kubectl patch ksvc test-app -n test-app -p '{"spec":{"template":{"metadata":{"annotations":{"autoscaling.knative.dev/minScale":"5"}}}}}'
```
...
```bash
# Backup ConfigMaps
kubectl get configmaps -n test-app -o yaml > configmaps-backup.yaml

# Backup Secrets (WARNING: Contains sensitive data!)
kubectl get secrets -n test-app -o yaml > secrets-backup.yaml
# Encrypt backup file
...
kubectl set image deployment/test-app user-container=localhost:5001/test-org/test-app:v2.0 -n test-app
...
kubectl exec -it deployment/test-app -n test-app -- sh
...
kubectl patch ksvc test-app -n test-app -p '{"spec":{"template":{"metadata":{"annotations":{"autoscaling.knative.dev/minScale":"0"}}}}}'

# Wait for pods to terminate
kubectl wait --for=delete pods -l serving.knative.dev/service=test-app -n test-app --timeout=300s
```
...
kubectl wait --for=condition=ready ksvc test-app -n test-app --timeout=300s

# Test health
kubectl exec deployment/test-app -n test-app -- curl localhost:8080/health/ready
```
...
```bash
# Gradual rollout
kubectl set traffic test-app \
  --revision=test-app-00001=90 \
  --revision=test-app-00002=10 \
  -n test-app
```
...
kubectl wait --for=delete pods -l serving.knative.dev/service=test-app -n test-app
```
...
```bash
# Stop application
kubectl scale deployment/test-app --replicas=0 -n test-app
